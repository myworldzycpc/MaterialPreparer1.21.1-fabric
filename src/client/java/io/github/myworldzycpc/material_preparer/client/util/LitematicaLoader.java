...
public class LitematicaLoader {
    ...
    private static void processRegion(CompoundTag region, Map<Block, Integer> blockCounts) {
        // 获取方块状态调色板
        ListTag palette = region.getList("BlockStatePalette").orElse(new ListTag()); // 10 = compound
        if (palette.isEmpty()) {
        ...
        for (int i = 0; i < palette.size(); i++) {
            CompoundTag entry = palette.getCompound(i).orElse(null);
            ...
            if (id != null && BuiltInRegistries.BLOCK.containsKey(id)) {
                ...
                if (ref == null) {
                    LOGGER.warn("Unknown block ID: {}", name);
                    blocks[i] = Blocks.AIR;
                } else {
                    blocks[i] = ref.value();
                }
        ...
        if (region.contains("Size")) {
            CompoundTag size = region.getCompound("Size").orElse(new CompoundTag());
            width = Math.abs(size.getInt("x").orElse(0));
            height = Math.abs(size.getInt("y").orElse(0));
            length = Math.abs(size.getInt("z").orElse(0));
        }
